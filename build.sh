...
# Only seed the default config when there is none. RetroArch rewrites this file
# itself on exit, so on a rebuild the upstream default would discard every
# setting the user has accumulated.
if [ ! -f "${SCRIPT_DIR}/retroarch.cfg" ]; then
